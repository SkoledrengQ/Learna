...
[Authorize]
[ApiController]
public sealed class FilesController(IFileResourceRepository files, IAnnouncementRepository announcements, IFileStorage storage, IUserRepository users, TimeProvider timeProvider, ILogger<FilesController> logger) : ControllerBase
{
    ...
    [HttpPost("api/announcements/{id:int}/files")]
    [RequestSizeLimit(262_144_000)]
    public async Task<ActionResult<FileResourceDto>> UploadToAnnouncement(int id, IFormFile file, [FromForm] string? description, CancellationToken cancellationToken)
    {
        var announcement = await files.GetAnnouncementAsync(id); if (announcement == null) return NotFound(); var user = await CurrentUserAsync();
        if (!User.IsInRole("Admin") && user.Id != announcement.CreatedByUserId) return Forbid();
        return await StoreAsync(file, description, user, null, null, cancellationToken, announcement: announcement);
    }
    ...
    [HttpGet("api/announcements/{id:int}/files")]
    public async Task<ActionResult<IEnumerable<FileResourceDto>>> ListAnnouncement(int id)
    {
        var announcement = await announcements.GetByIdAsync(id); if (announcement == null) return NotFound(); var user = await CurrentUserAsync();
        if (!await AnnouncementAccess.CanSeeAsync(announcement, user, User.IsInRole("Admin"), announcements, Now)) return Forbid();
        return Ok((await files.GetForAnnouncementAsync(id)).Select(f => ToDto(f, user)));
    }
    ...
    public async Task<IActionResult> Delete(int id, CancellationToken cancellationToken)
    {
        ...
        if (file.AnnouncementId.HasValue && !User.IsInRole("Admin") && user.Id != file.UploadedByUserId && user.Id != file.Announcement?.CreatedByUserId) return Forbid();
        if (!file.AnnouncementId.HasValue && !User.IsInRole("Admin") && user.Id != file.UploadedByUserId && (!user.TeacherId.HasValue || user.TeacherId != targetTeacherId)) return Forbid();
    ...
    private async Task<ActionResult<FileResourceDto>> StoreAsync(IFormFile upload, string? description, User user, SubjectGroup? group, Lesson? lesson, CancellationToken cancellationToken, Assignment? assignment = null, Announcement? announcement = null)
    {
        ...
        entity.UploadedByUser = user; entity.SubjectGroup = group; entity.Lesson = lesson; entity.Assignment = assignment; entity.Announcement = announcement;
    ...
    private bool CanDelete(FileResource file, User user) => User.IsInRole("Admin") || user.Id == file.UploadedByUserId || user.Id == file.Announcement?.CreatedByUserId || (user.TeacherId.HasValue && user.TeacherId == (file.Lesson?.TeacherId ?? file.SubjectGroup?.TeacherId ?? file.Assignment?.SubjectGroup.TeacherId ?? file.Submission?.Assignment.SubjectGroup.TeacherId));
    ...
    private async Task<bool> CanDownloadAsync(FileResource file, User user)
    {
        ...
        if (file.Announcement is { } announcement) return await AnnouncementAccess.CanSeeAsync(announcement, user, false, announcements, Now);
