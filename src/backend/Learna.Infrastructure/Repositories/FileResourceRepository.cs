...
public sealed class FileResourceRepository(ApplicationDbContext context) : IFileResourceRepository
{
    ...
    {
        if (isAdmin) return (await Files.OrderByDescending(f => f.CreatedAt).ToListAsync()).Select(f => new VisibleFileResource(f, null, null)).ToList();
        if (user.StudentId is int studentId)
        {
            ...
            return (await Files.Where(f => groupIds.Contains(f.SubjectGroupId ?? f.Lesson!.SubjectGroupId)).OrderByDescending(f => f.CreatedAt).ToListAsync()).Select(f => new VisibleFileResource(f, null, null)).ToList();
        }
        if (user.GuardianId is int guardianId)
        {
            ...
            {
                var files = await Files.Where(f => f.SubjectGroupId == row.SubjectGroupId || (f.LessonId != null && f.Lesson!.SubjectGroupId == row.SubjectGroupId)).ToListAsync();
        ...
        if (user.TeacherId is int teacherId)
            return (await Files.Where(f => f.SubjectGroup!.TeacherId == teacherId || f.Lesson!.TeacherId == teacherId).OrderByDescending(f => f.CreatedAt).ToListAsync()).Select(f => new VisibleFileResource(f, null, null)).ToList();
