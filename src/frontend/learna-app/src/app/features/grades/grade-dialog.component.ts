...
</mat-dialog-content><mat-dialog-actions align="end"><button mat-button mat-dialog-close>{{'common.cancel'|transloco}}</button><button mat-raised-button color="primary" [disabled]="!data.category.trim()||(!data.allowCategory&&(data.score===null||data.score<0||data.maxScore<=0||data.score>data.maxScore))" (click)="save()">{{'common.save'|transloco}}</button></mat-dialog-actions>`,styles:[`mat-form-field{display:block}.scores{display:flex;align-items:center;gap:12px}.scores mat-form-field{width:150px}`]})
