      ...
      <mat-tab-group>
        ...
        <mat-tab [label]="'assignments.title'|transloco"><section class="assignments">
          ...
          <section class="submissions" *ngIf="activeAssignment() as active"><h2>{{'assignments.submissionsFor'|transloco:{title:active.title} }}</h2>
            ...
            <div class="actions"><button mat-raised-button color="primary" (click)="confirmPublishGrades(active)">{{'grades.publishAll'|transloco}}</button></div>
            <article class="student" *ngFor="let row of filteredRoster()"><div><strong>{{row.studentName}}</strong> <small>{{row.studentNumber}}</small><mat-chip>{{('assignments.filters.'+row.status)|transloco}}</mat-chip><mat-chip [class.published]="row.submission?.grade?.status==='Published'">{{('grades.states.'+(row.submission?.grade?.status||'Ungraded'))|transloco}}</mat-chip><p *ngIf="row.submission?.text">{{row.submission?.text}}</p><button mat-button *ngFor="let file of row.submission?.files" (click)="materials.download(file)"><mat-icon>download</mat-icon>{{file.originalFileName}}</button></div><div><button mat-button *ngIf="row.submission" (click)="editSubmissionGrade(row)">{{'grades.grade'|transloco}}</button><button mat-button *ngIf="row.submission?.grade?.status==='Draft'" (click)="publishGrade(row.submission!.grade!)">{{'grades.publish'|transloco}}</button><button mat-button (click)="beginExtension(row)">{{'assignments.grantExtension'|transloco}}</button></div></article>
